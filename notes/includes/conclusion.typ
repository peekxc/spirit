...
*Function parameterization*: On the spectral side, in particular, tuning the degree of regularization of the chosen matrix function @eq:lowner can be very application-dependent. We demonstrate this in @fig:codensity_opt ---for some values of $tau > 0$, the relaxation can be either have high variance due to instability or it can exhibit high bias due over smoothing. 
In the optimization setting, we addressed these issues via spectral regularization and iterative shrinkage, however other methods could also be used.
The reason this phenomenon occurs in general is due to the combinatorial nature of the rank invariant itself: although the rank invariant is often called a "stable function" even in the multi-parameter setting @cerri2013betti, this stability exists *only* in the pairings themselves. Most notably, the _Betti sequence_---which is the sequence containing the Betti numbers of the homology groups at all scales---is *unstable* with respect to the $1$-Wasserstein distance (see Theorem 2.5 in @johnson2021instability).

*Randomized estimation:* In practice, the efficiency of the iterative implicit trace estimator proposed in @sec:iterative_approx depends on a variety of factors. 
For some of the persistence invariants---like the pairing itself---a single wrong rank value in the divide-and-conquer algorithm from @chen2011output can lead to unpredictable output. 
Thus, if the invariant to be computed needs to be correct with high confidence and the randomized estimator has success probability $p > 0$, then one may need to be re-run $k$ times to achieve a higher success probability $(1 - (1-p)^k)$. 
...
Indeed, the analysis from section 6 of @chen2011output shows that is the rank estimator has success probability $p$ and $r = 1 - p$, then for the entire algorithm to achieve an arbitrary success probablity $q > 0$, every rank computation needs to be repeated at most $k$ times, where: 

$ log(1 slash r)^(-1) dot log (omega) <= k, quad omega = (1 - q^(1/rho))^(-1) $

Since $log(omega) = Theta(rho)$, we have that every rank computation requires $O(log rho) = O(log n + log 1 / delta)$ repeated trials. In other words, the randomized aspect of the algorithm does not signicantly affect its scalability. 

*Estimator efficiency:* The primary limitation of approximating the rank function via the GH Monte-carlo estimator via @eq:gh_trace_estimator is its convergence, which is dominated by $O(epsilon.alt^(-2))$. If $epsilon.alt$ is small, the number of iterations the estimator needs can be enormous. 
Thus its practical utility lies in applications where $epsilon.alt$ need not be too small, i.e. only a relatively coarse approximation of $tr (f (A))$ is needed. 
Unfortunately, estimating the rank exactly requires shrinking $epsilon.alt$ down to a factor of $O (1 slash n)$. 
// which itself is implies the $lr((1 plus.minus epsilon.alt))$-approximation 
// may require upwards of 
This combination of slow convergence and high precision is the primary limitation of the @eq:gh_trace_estimator in practical persistence settings, and to us to the only barrier preventing widespread adoption.

Fortunately, implicit trace estimation is active area of research. Recent results by Meyers et al. @meyer2021hutchpp, for example, show how the estimator @eq:gh_trace_estimator can be modified to obtain a $lr((1 plus.minus epsilon.alt))$ approximation using only $O( epsilon^(-1) dot sqrt( log (eta^(-1))) + log (eta^(-1)))$ samples using deflation techniques, though this does require additional re-orthogonalization and storage costs. Even more recently, optimal trace estimators based on the _exchangeability principle_ and the _Nyström approximation_ have been shown to not only match the $O(1 slash n^2)$ variance reduction rate Hutch++, but to also empirically achieve much faster convergence @epperly2024xtrace. 
